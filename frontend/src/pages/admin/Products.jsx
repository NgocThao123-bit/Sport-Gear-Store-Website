// admin/Products.jsx — admin product management with create + edit modal
// admin/Products.jsx — quản lý sản phẩm admin với modal tạo + sửa sản phẩm
...
// ProductDto now includes categoryId, description, isActive from backend
const formFromProduct = (p) => ({
  ...
  isActive:    p.isActive,
  // edit mode does NOT change images or variants (backend UpdateProduct doesn't support it)
  imageUrls:   [''],
  variants:    [{ size: '', color: '', stock: 10, extraPrice: 0 }],
});
...
export default function AdminProducts() {
  ...
  const openEdit = (product) => {
    ...
    setFormError('');
    setModal(product);   // store the full product so we have its id
  };
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      ...
      // If we deleted the last item on this page, jump back one page
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isEdit) {
      ...
      if (!validImages.length) return setFormError('At least one image URL is required.');
    }
    ...
    try {
      if (isEdit) {
        // PUT /api/products/{id}
        // UpdateProductCommand: { id, name, brand, description, price, salePrice, categoryId, isActive }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex items-end justify-between mb-8">
          <div>
            ...
            <p className="text-brand-ink/50 text-sm mt-1">{products.length} products total</p>
          </div>
          <button
            onClick={openCreate}
            ...
            <table className="w-full text-sm">
              <thead className="bg-brand-ink text-brand-lime">
                <tr>
                  ...
                  <th className="text-left px-6 py-4 font-display tracking-widest">BRAND</th>
                    ...
                    <td className="px-4 py-2">
                      {p.mainImageUrl ? (
                        <img
                          src={p.mainImageUrl}
                          alt={p.name}
                          className="w-12 h-12 object-cover rounded-xl border border-brand-ink/10"
                        />
                      ) : (
                        <div className="w-12 h-12 rounded-xl bg-brand-ink/10 flex items-center justify-center text-brand-ink/30 text-xs">
                          N/A
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-4 font-bold text-brand-ink">{p.name}</td>
                    <td className="px-6 py-4 text-brand-ink/60 text-xs uppercase tracking-widest">{p.brand}</td>
                    <td className="px-6 py-4 font-display">
                      {p.salePrice ? (
                        <span className="flex items-baseline gap-2">
                          <span className="text-brand-purple">${p.salePrice.toFixed(2)}</span>
                          <span className="line-through text-brand-ink/30 text-xs">${p.price.toFixed(2)}</span>
                        </span>
                      ) : (
                        <span className="text-brand-purple">${p.price?.toFixed(2)}</span>
                      )}
                    </td>
                    <td className="px-6 py-4 text-brand-ink/60">{p.categoryName}</td>
                    <td className="px-6 py-4">
                      <span className={`text-xs font-bold px-2 py-1 rounded-full ${p.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-500'}`}>
            ...
            <form onSubmit={handleSubmit} className="px-8 py-6 flex flex-col gap-6">
              ...
              {!isEdit && (
                <div className="flex flex-col gap-2">
                  ...
                  <button type="button" onClick={addImage} className="self-start text-xs font-bold tracking-widest uppercase text-brand-purple hover:text-brand-ink">
                    + Add Image
                  </button>
                </div>
              )}
              ...
              {!isEdit && (
                <div className="flex flex-col gap-2">
                  ...
                  <button type="button" onClick={addVariant} className="self-start text-xs font-bold tracking-widest uppercase text-brand-purple hover:text-brand-ink">
                    + Add Variant
                  </button>
                </div>
              )}
              ...
              {isEdit && (
                <p className="text-xs text-brand-ink/40 italic">
                  Note: images and variants cannot be changed here. Use the API directly for those.
                </p>
              )}

              {/* Error */}
              {formError && (
                <p className="text-red-500 text-sm font-bold border border-red-200 bg-red-50 rounded-xl px-4 py-2">
                  {formError}
                </p>
              )}
